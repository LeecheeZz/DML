...
parser.add_argument('--name', default='convnext_se2_LPN2_384_SUES150', type=str, help='save model path')
...
yaml.warnings({'YAMLLoadWarning': False}) # 告诉YAML解析器不要发出名为"YAMLLoadWarning"的警告
...
def extract_feature(model,dataloaders, view_index = 1):
    ...
    for data in tqdm(dataloaders):
        ...
        for i in range(2): # 0, 1
            ...
            for scale in ms:
                if scale != 1:
                    # bicubic is only  available in pytorch>= 1.1  对输入的图像进行按照指定的尺度因子进行双线性插值操作，从而实现图像的放大或缩小
                    input_img = nn.functional.interpolate(input_img, scale_factor=scale, mode='bilinear', align_corners=False)
                if opt.views ==2:
                    if view_index == 1:
                        outputs, _ = model(input_img, None) # outputs : [8, 512, 6]
                    elif view_index ==3:
                ...
                if i==0:
                    ff = outputs # ff : [8, 512, 6]
                else:
                    ff += outputs # ff : [8, 512, 6]

        # norm feature
        if len(ff.shape)==3: # ff.shape : [8, 512, 6]
            fnorm = torch.norm(ff, p=2, dim=1, keepdim=True) * np.sqrt(ff.size(-1))
            ff = ff.div(fnorm.expand_as(ff))
            ff = ff.view(ff.size(0), -1) # [8, 3072]
        else:
...
print('-------test-----------')
with open(os.path.join('model',opt.name,opt.fname),'a',encoding='utf-8') as f: # model/convnext_se2_ratio8/test.txt
    text = str(('-------test-----------'))+'\n'
    f.write(text)
model, _, epoch = load_network(opt.name, opt)
model.head = nn.Sequential() # 头部通常指的是最后的几层，这些层负责将神经网络的中间表示（或者特征）转换为最终的输出结果
# print(model)
with open(os.path.join('model',opt.name,opt.fname),'a',encoding='utf-8') as f: # model/convnext_se2_ratio8/test.txt
    text = str(model)+'\n'
    f.write(text)
...
print('%d -> %d:'%(which_query, which_gallery)) # 1 -> 3
with open(os.path.join('model',opt.name,opt.fname),'a',encoding='utf-8') as f:
    text = str(('%d -> %d:'%(which_query, which_gallery)))+'\n'
    f.write(text)

gallery_path = image_datasets[gallery_name].imgs
f = open(os.path.join('model',opt.name,'gallery_name.txt'),'w') # 'model/convnext_se2_ratio8/gallery_name.txt'
for p in gallery_path:
    f.write(p[0]+'\n')
query_path = image_datasets[query_name].imgs
f = open(os.path.join('model',opt.name,'query_name.txt'),'w') # 'model/convnext_se2_ratio8/query_name.txt'
for p in query_path:
    f.write(p[0]+'\n')
