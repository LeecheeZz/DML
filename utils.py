...
def get_model_list(dirname, key): # ./model/model/convnext_se2_ratio8
    if os.path.exists(dirname) is False:
        print('no dir: %s'%dirname)
        return None
    gen_models = [os.path.join(dirname, f) for f in os.listdir(dirname) if
                  os.path.isfile(os.path.join(dirname, f)) and key in f and ".pth" in f] #['./model/model/convnext_se2_ratio8/net_090.pth', ...]
    if gen_models is None:
        return None
    gen_models.sort()
    last_model_name = gen_models[-1]
    return last_model_name # 返回最后一个模型
...
def load_network(name, opt):
    # Load config
    dirname = os.path.join('./model',name) # './model/model/convnext_se2_ratio8'
    last_model_name = os.path.basename(get_model_list(dirname, 'net')) # './model/model/convnext_se2_ratio8/net_164.pth'   'net_164.pth'
    epoch = last_model_name.split('_')[1] # '164.pth'
    epoch = epoch.split('.')[0] # '164'
    if not epoch=='last':
       epoch = int(epoch) # 164
    config_path = os.path.join(dirname,'opts.yaml') # './model/model/convnext_se2_ratio8/opts.yaml'
    ...
    save_path = os.path.join('./model',name,save_filename) # './model/model/convnext_se2_ratio8/net_164.pth'
