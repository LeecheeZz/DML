...
from losses.cal_loss import cal_kl_loss,cal_loss,cal_triplet_loss,cal_infoNCE_loss,cal_dkd_loss, DKD
...
parser.add_argument('--name',default='ConvNeXt_SSE_LPN_384_Albumentataions_DKD_SUES150', type=str, help='output model name')
parser.add_argument('--data_dir',default='../University1652-Baseline-master/data/train',type=str, help='training dir path')                                                 
...
parser.add_argument('--kl_loss', action='store_true',default=True, help='kl_loss' )
...
y_loss = {} # loss history
y_loss['train'] = [] # {'train' : [], 'val' : []}
y_loss['val'] = []
y_err = {}
y_err['train'] = [] # {'train' : [], 'val' : []}
y_err['val'] = []
...
def train_model(model, opt, model_test, optimizer, scheduler, num_epochs=25):
    ...
    warmup_dkd = 60
    criterion_dkd = DKD(alpha=0.4, beta=3.2, temperature=4)
        ...
        for phase in ['train']:
            ...
            for data,data3 in dataloaders:
                ...
                if phase == 'val':
                    ...
                    if opt.autocast:
                        with autocast(): #是PyTorch中用于自动混合精度计算的上下文管理器,对于一些不需要高精度计算的操作（例如前向传播、梯度计算等），会自动转换为半精度计算，在使用with autocast():时，通常需要结合PyTorch的torch.cuda.amp.GradScaler类来自动调节梯度的精度，以确保模型训练的稳定性和收敛性
                            if opt.views == 2:
                ...
                if opt.triplet_loss > 0: # 0.3
                    # features = outputs[1] # [[8, 512], [8, 512], [8, 512]]
                    # features2 = outputs2[1] # [[8, 512], [8, 512], [8, 512]]
                    # split_num = opt.batchsize//opt.sample_num # 8 // 1
                    # f_triplet_loss = cal_triplet_loss(features,features2,labels,triplet_loss,split_num)

                    outputs = outputs[0] # [[8, 701], [8, 701], [8, 701]]
                    outputs2 = outputs2[0] # [[8, 701], [8, 701], [8, 701]]

                if isinstance(outputs,list):
                    preds = []
                    preds2 = []
                    for out,out2 in zip(outputs,outputs2):
                        preds.append(torch.max(out.data,1)[1]) # tensor([ 52,  32, 162, 695, 459, 199, 365, 637], device='cuda:0')
                ...
                if opt.views == 2:
                    cls_loss = cal_loss(outputs, labels, criterion) + cal_loss(outputs2, labels3, criterion) # CrossEntropyLoss
                    if opt.kl_loss:
                        # kl_loss = cal_kl_loss(outputs, outputs2, loss_kl) # KL散度损失
                        #-----------------------------------------------------------------------
                        #------------------------------DMLLoss----------------------------------
                        #-----------------------------------------------------------------------
                        kl_loss =  min(epoch / warmup_dkd, 1.0) * cal_dkd_loss(outputs, outputs2, labels, criterion_dkd)
                elif opt.views == 3:
                    outputs3 = outputs3[0] # [[8, 701], [8, 701], [8, 701]]
                    if isinstance(outputs, list):
                        preds3 = []
                        for out3 in outputs3:
                            preds3.append(torch.max(out3.data, 1)[1]) # 输出的最大分数的index
            ...
            if epoch >= 120 and epoch_loss < min_loss:
                save_network(model, opt.name, epoch)
                min_loss = epoch_loss
            print(y_loss['val'])
            y_loss['train']
