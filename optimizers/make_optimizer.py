...
def make_optimizer(model,opt):
    ...
    if opt.views==3:
        ...
        for i in [model.model_1]:
            ignored_params += list(map(id, i.convnext.parameters()))
    extra_params = filter(lambda p: id(p) not in ignored_params, model.parameters()) # 需要更新的参数
    base_params = filter(lambda p: id(p) in ignored_params, model.parameters()) # 忽略的参数
    optimizer_ft = optim.SGD([
        {'params': base_params, 'lr': 0.3 * opt.lr}, # lr : 0.01
